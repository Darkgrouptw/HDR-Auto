...
typedef double  COLOR[3];        /* red, green, blue */
...
template<class T>
int fwritescan(T *scanlineR, T *scanlineG, T *scanlineB, int len, FILE *fp) {              /* write out a scanline */
    COLR *clrscan;
    int  n;
    COLR *sp;
    /* get scanline buffer */
    ...
    n = len;
    while (n-- > 0) {
        setcolr(sp[0], (double)scanlineR[0], (double)scanlineG[0], (double)scanlineB[0]);
        scanlineR++, scanlineG++, scanlineB++;
        sp++;
    }
    return(fwritecolrs(clrscan, len, fp));
}
...
/* 
 * Write an HDR radiance image 
 */  
  
void write_radiance (char *outfilename, COLOR **rgb_image, int width, int height)  
{  
  FILE *f = fopen(outfilename, "wb");
  
  // Write trivial hdr header
  fprintf(f,"#?RADIANCE\n");
  fprintf(f,"32-bit_rle_rgbe\n");
  fprintf(f,"\n");
  fprintf(f,"-Y %d +X %d\n", height, width);
  
  for (int y = 0; y < height; y++)  
    fwritescan (rgb_image[y], width, f);  
